...
public class Deal extends PublicIdEntity {
  ...
  private DealCancelReason cancelReason;
  ...
  @Getter
  @Column(name = "canceled_after_contact_reveal", nullable = false)
  private boolean canceledAfterContactReveal;
  ...
  public void markContactShared() {
    requireDealStatus(DealStatus.ACTIVE, DealStatus.WORK_SUBMITTED, DealStatus.REVISION_REQUESTED);
    this.contactVisibilityStatus = ContactVisibilityStatus.SHARED;
  }

  public void cancelByParticipant(
      User actor,
      DealCancelReason reason,
      String comment,
      boolean afterContactReveal,
      Instant canceledAt) {
    requireDealStatus(DealStatus.ACTIVE, DealStatus.REVISION_REQUESTED);
    this.status = DealStatus.CANCELED;
    ...
    this.canceledAfterContactReveal = afterContactReveal;
    this.canceledAt = canceledAt;
  }
  ...
  private void requireDealStatus(DealStatus... allowedStatuses) {
    ...
    throw new InvalidStatusTransitionException(
        "Deal", this.status.name(), allowedStatuses[0].name());
  }
}
