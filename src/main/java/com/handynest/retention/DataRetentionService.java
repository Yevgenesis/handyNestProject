...
public class DataRetentionService {
  ...
  public DataRetentionRunResult runRetention(Instant now) {
    ...
    long anonymizedUsers = anonymizeDeletedUsers(now, policy.deletedUserAnonymizationDays());
    long expiredVerificationDocuments =
        expireRejectedVerificationDocuments(now, policy.verificationDocumentRetentionDays());
  ...
  private long anonymizeDeletedUsers(Instant now, int retentionDays) {
    ...
    List<User> users =
        userRepository.findAllByStatusAndDeletedAtBefore(
            UserStatus.DELETED,
            deletedBefore,
  ...
  private long expireRejectedVerificationDocuments(Instant now, int retentionDays) {
    ...
    List<MarketplaceAttachment> documents =
        marketplaceAttachmentRepository.findRejectedVerificationDocumentsForRetention(
            reviewedBefore, PageRequest.of(0, dataRetentionProperties.getBatchSize()));
  ...
  private RetentionPolicySnapshot activePolicy() {
    return dataRetentionPolicyRepository
  ...
  private record RetentionPolicySnapshot(
      int verificationDocumentRetentionDays, int deletedUserAnonymizationDays) {}
}
