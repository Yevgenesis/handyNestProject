...
public class RiskDetectionService {
    ...
    private static final Pattern PAYMENT_OUTSIDE_PLATFORM_PATTERN = Pattern.compile(
            "(?iu)(оплат[ауы]?\\s+вне\\s+платформы|перевед[иі]|на\\s+карту|kaspi|каспи)"
    );
    private static final Pattern EXTERNAL_MESSENGER_PATTERN = Pattern.compile(
            "(?iu)(telegram|whatsapp|wa\\.me|t\\.me|instagram|\\bватсап\\b|\\bтелеграм\\b)"
    );
    private static final Pattern PHONE_PATTERN = Pattern.compile(
            "(?iu)(?:\\+?7|8)?[\\s\\-()]*\\d{3}[\\s\\-()]*\\d{3}[\\s\\-()]*\\d{2}[\\s\\-()]*\\d{2}"
    );
    private static final Pattern EMAIL_PATTERN = Pattern.compile(
            "(?iu)[A-Z0-9._%+-]+@[A-Z0-9.-]+\\.[A-Z]{2,}"
    );
    ...
    public Optional<RiskDetectionResult> detect(String text) {
        ...
        return firstMatch(text, PAYMENT_OUTSIDE_PLATFORM_PATTERN, RiskType.PAYMENT_OUTSIDE_PLATFORM, RiskSeverity.HIGH)
                .or(() -> firstMatch(text, EXTERNAL_MESSENGER_PATTERN, RiskType.EXTERNAL_MESSENGER, RiskSeverity.HIGH))
                ...
                .or(() -> firstMatch(text, EXTERNAL_LINK_PATTERN, RiskType.EXTERNAL_LINK, RiskSeverity.MEDIUM));
    }
    ...
    private Optional<RiskDetectionResult> firstMatch(
        ...
        return Optional.of(new RiskDetectionResult(
                riskType,
                severity,
                detectedText,
                normalize(detectedText)
        ));
    }
