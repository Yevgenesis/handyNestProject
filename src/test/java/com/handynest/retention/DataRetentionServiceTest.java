...
class DataRetentionServiceTest {
  ...
  @Mock private MarketplaceAttachmentRepository marketplaceAttachmentRepository;
  ...
  void runRetentionAnonymizesDeletedUsersAndExpiresRejectedVerificationDocuments() {
    ...
    when(userRepository.findAllByStatusAndDeletedAtBefore(
            eq(UserStatus.DELETED), any(Instant.class), any(Pageable.class)))
    ...
    assertThat(deletedUser.getEmail())
        .isEqualTo("deleted+01kaccountdeleted0000000001@deleted.handynest.local");
